...
export async function POST(req: Request) {
  try {
    ...
    (r as any).headers = Object.fromEntries(
      req.headers.entries ? req.headers.entries() : req.headers
    );
    ...
    const saved: { field: string; filename: string; path: string; size: number }[] = [];
    ...
    for (const [field, fileVal] of Object.entries(files || {})) {
      ...
      if (Array.isArray(fileVal)) {
        for (const f of fileVal) {
          saved.push({
            field,
            filename: path.basename(f.filepath || f.path || ''),
